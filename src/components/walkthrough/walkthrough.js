  ...
  connectedCallback() {
    if (!this.shadowRoot && typeof window !== 'undefined') {
      ...
      template.innerHTML = `
        ...
        <ol>
          ${Array.from(this.cards).map((card, idx) => {      
    ...
    // this.shadowRoot.adoptedStyleSheets = [ theme ];
    this.shadowRoot?.querySelectorAll('li')?.forEach(item => item.addEventListener('click', this.selectItem.bind(this)))
  }

  selectItem(event) {
    console.log('selectItem', event.currentTarget.dataset.idx);
    this.index = event.currentTarget.dataset.idx;
    this.shadowRoot.querySelector('h3').innerHTML = this.cards[this.index].querySelector('p').innerHTML;
    this.shadowRoot.querySelector('.snippet').innerHTML = this.cards[this.index].querySelector('pre').innerHTML;
  }
}
