  ...
  acornWalk.simple(acorn.parse(scriptContents, {
    ...
    ImportDeclaration(node) {
      ...
      if(value.endsWith('.module.css') && specifiers.length === 1 && specifiers[0].local.name === 'styles') {
        ...
        const cssModulesMap = JSON.parse(fs.readFileSync(new URL('./__css-modules-map.json', compilation.context.scratchDir)));
        console.log('UPDATE MAP!', cssModulesMap)
        fs.writeFileSync(new URL('./__css-modules-map.json', compilation.context.scratchDir), JSON.stringify({
          ...cssModulesMap,
          [`${cssModuleUrl.href}`]: {
            module: classNameMap,
        ...
        //   contents: scopedCssContents
        // })
        console.log('after update', JSON.parse(fs.readFileSync(new URL('./__css-modules-map.json', compilation.context.scratchDir))));
...
class CssModulesResource extends ResourceInterface {
  constructor(compilation, options) {
    ...
    if(!fs.existsSync(this.compilation.context.scratchDir.pathname)) {
      ...
      fs.writeFileSync(new URL('./__css-modules-map.json', this.compilation.context.scratchDir).pathname, JSON.stringify({}));
    }
  }
  ...
  async shouldServe(url) {
    ...
    const cssModulesMap = JSON.parse(fs.readFileSync(new URL('./__css-modules-map.json', this.compilation.context.scratchDir), 'utf-8'));
    return (url.protocol === 'file:' && url.pathname.endsWith(this.extensions[0]) && cssModulesMap[url.href]);
  }

  async serve(url) {
    // console.log(url, globalThis.cssModulesMap)
    const cssModulesMap = JSON.parse(fs.readFileSync(new URL('./__css-modules-map.json', this.compilation.context.scratchDir)));
